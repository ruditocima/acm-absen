...
function renderRekapDataToTable(dataList) {
    ...
    tbody.innerHTML = dataList.map(function(r, i) {
        return '<tr class="hover:bg-slate-900/50">' +
            ...
            '<td class="p-3">' + (r.selfie_url ? '<button onclick="openImageZoom('' + r.selfie_url + '')" class="px-2 py-1 bg-slate-800 text-gold-400 border border-slate-700 hover:bg-slate-700 rounded text-[11px] font-semibold transition flex items-center gap-1"><i class="fa-solid fa-image"></i> Lihat</button>' : '-') + '</td></tr>';
    }).join('');
}
    ...
    if (myRekap.length === 0 && myIzin.length === 0) {
        ...
        myRekap.slice().reverse().forEach(function(r) {
            html += '<div class="bg-slate-950/40 p-3 rounded-xl border border-slate-800 flex justify-between items-center mb-2">' +
                '<div><p class="text-xs font-bold text-white">' + r.date + '</p><p class="text-[10px] text-slate-400"><i class="fa-solid fa-location-dot text-gold-400"></i> ' + r.basecamp + '</p></div>' +
                '<div class="text-right"><p class="text-xs font-mono text-emerald-400">' + r.time + ' WIB</p><p class="text-[10px] ' + (r.status === 'Tepat Waktu' ? 'text-emerald-500' : 'text-amber-500') + ' font-semibold">' + r.status + '</p></div></div>';
        });
        myIzin.slice().reverse().forEach(function(i) {
            html += '<div class="bg-slate-950/40 p-3 rounded-xl border border-slate-800 flex justify-between items-center mb-2">' +
                '<div><p class="text-xs font-bold text-white">Izin: ' + i.jenis + '</p><p class="text-[10px] text-slate-400">' + i.start + ' s/d ' + i.end + '</p></div>' +
                '<div class="text-right"><span class="px-2 py-0.5 rounded text-[9px] font-bold ' + (i.status === 'Approved' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400') + '">' + i.status + '</span></div></div>';
        });
    }
